...
bool CField::AssertValid(CFieldWorld& world) const
{
	...
	{	// m_dim_coord检查
		...
		// TODO : bubble , edge我也想了解一下
	}
	{	// corner value是否匹配表示是否是部分字段的标志
		if (m_na_c.id_na_va != 0) {
...
INTERPOLATION_TYPE CField::GetInterpolationType(unsigned int id_ea, const CFieldWorld& world) const {
	...
	if (elem_type == LINE) {
		if (ei.id_es_c_va != 0 && ei.id_es_b_va == 0 && ei.id_es_e_va == 0) { return LINE11; }	// 捈慄堦師曗娫
		else {
		...
		if (ei.id_es_c_va != 0 && ei.id_es_b_va == 0 && ei.id_es_e_va == 0) { return TRI11; }	// 嶰妏宍堦師曗娫
		else if (ei.id_es_c_va != 0 && ei.id_es_b_va != 0 && ei.id_es_e_va == 0) { return TRI1011; }	// 嶰妏宍僶僽儖曗娫(僒僽僷儔儊僩儕僢僋)
		else if (ei.id_es_c_va == 0 && ei.id_es_b_va != 0 && ei.id_es_e_va == 0) { return TRI1001; }	// 嶰妏宍梫慺堦掕曗娫
		else {
			std::cout << "Error!-->Interpolation Not Defined(Tri)" << std::endl;
			std::cout << ei.id_es_c_va << " " << ei.id_es_b_va << std::endl;
			assert(0);
		...
		if (ei.id_es_c_va != 0 && ei.id_es_b_va == 0 && ei.id_es_e_va == 0) { return TET11; }	// 巐柺懱堦師曗娫
		else if (ei.id_es_c_va == 0 && ei.id_es_b_va != 0 && ei.id_es_e_va == 0) { return TET1001; }	// 巐柺懱梫慺堦掕曗娫
	}
	else if (elem_type == HEX) {
		if (ei.id_es_c_va != 0 && ei.id_es_b_va == 0 && ei.id_es_e_va == 0) { return HEX11; }	// 榋柺懱堦師曗娫
		else if (ei.id_es_c_va == 0 && ei.id_es_b_va != 0 && ei.id_es_e_va == 0) { return HEX1001; }	// 榋柺懱梫慺堦掕曗娫
	}
...
// TODO: 堦搙偙偺娭悢傪屇傫偩傜丆師偵屇傫偩帪偼崅懍偵張棟偝傟傞傛偆偵丆僴僢僔儏傪峔抸偡傞
// TODO: 嵗昗傗僐僱僋僥傿價僥傿偺曄峏偑偁偭偨応崌偼丆僴僢僔儏傪嶍彍偡傞
