...
void Cad::CEdge2D::GetBoundingBox( double& x_min, double& x_max, double& y_min, double& y_max ) const
{
	...
	if( itype == 0 ){ return; }	// 捈慄偺応崌偼偦偺傑傑
	else if( itype == 1 ){	// 墌屖偺応崌
		Com::CVector2D po_c;
		double radius;
...
bool Cad::CEdge2D::IsCrossEdge_ShareOnePoint(const CEdge2D& e1, bool is_share_s0, bool is_share_s1) const
{
  ...
  if(  is_share_s0 &&  is_share_s1 ){ assert( Com::SquareLength(po_s,po_s1) < 1.0e-20 ); }
	...
	else if( this->itype == 1 && e1.itype == 1 ){	// 墌屖摨巑偺岎嵎
		Com::CVector2D po_c0;
		double radius0;
		this->GetCenterRadius(po_c0,radius0);
		////////////////
	...
	else if( this->itype == 0 && e1.itype == 2 ){	// 捈慄偲愜傟慄偺岎嵎
		const std::vector<double>& relcomsh1 = e1.aRelCoMesh;
		const unsigned int ndiv1 = relcomsh1.size()/2+1;
	...
	else if( this->itype == 1 && e1.itype == 2 ){	// 墌屖偲愜傟慄偺岎嵎
		Com::CVector2D po_c0;
		double radius0;
		this->GetCenterRadius(po_c0,radius0);
		////////////////
	...
	else if( this->itype == 2 && e1.itype == 2 ){	// 愜傟慄摨巑偺岎嵎
		const Com::CVector2D& h0 = po_e-po_s;
		const Com::CVector2D v0(-h0.y,h0.x);
		const Com::CVector2D& h1 = e1.po_e-e1.po_s;
		const Com::CVector2D v1(-h1.y,h1.x);
...
{
    aCo.clear();
    if( ndiv <= 0 ){    // 弌棃傞偩偗揰懡偔偟偰拤幚側嬋慄偺昞尰
        if(      this->itype == 0 ) // 捈慄偺応崌
        {
            return true;
        }
        else if( this->itype == 1 ) // 墌屖偺応崌
        {
            double radius, theta;
            Com::CVector2D pc, lx,ly;
            this->GetCenterRadiusThetaLXY(pc,radius, theta,lx,ly);
            const unsigned int ndiv = (unsigned int)( theta*360/(6.0*6.28) ) + 1; // 6搙偛偲偺暘妱
            // 嵞婣屇傃弌偟偺偲偒偼ndiv=0偼愨懳擖傟偪傖偩傔(柍尷儖乕僾)
            return this->GetCurve_Mesh(aCo,ndiv);
        }
        else if( this->itype == 2 ) // 儊僢僔儏偺応崌
        {
...
// 墌屖偲曈(捈慄偱側偗傟偽側傜側偄)偺岎揰傪媮傔傞
// 俀偮偁傞応崌偼岎揰偺pos偐傜poe傊偺僷儔儊乕僞偑t1,t2偵擖傝丄曉傝抣偑侾偲側傞
// 岎揰偑柍偄応崌偼侽偑曉傝抣偲側傞
...
// v0偐傜僄僢僕偵増偭偨嫍棧偱len偺挿偝偵偁傞揰傪摼傞丏
// is_front==true 増側傜僄僢僕偵増偭偰丆is_front==false側傜僄僢僕偵増傢側偄
    ...
    //		e_i.GetBoundingBox(x_min_i,x_max_i, y_min_i,y_max_i);
		...
		for(unsigned int jedge=iedge+1;jedge<nedge;jedge++){
			...
			if( (ipo0-jpo0)*(ipo0-jpo1)*(ipo1-jpo0)*(ipo1-jpo1) != 0 ){
				// BoundingBox傪梡偄偰丆岎嶖偟側偄僷僞乕儞傪彍奜
				// edge_j偺僶僂儞僨傿儞僌儃僢僋僗傪庢摼
        ...
        if( bb_j.y_min > bb_i.y_max || bb_j.y_max < bb_i.y_min ) continue;
				////////////////
        //				if( x_min_j > x_max_i || x_max_j < x_min_i ) continue;	// 岎嶖偑偁傝偊側偄僷僞乕儞傪彍奜
        //				if( y_min_j > y_max_i || y_max_j < y_min_i ) continue;	// 忋偵摨偠
				// 岎揰偑柍偄偐敾抐偡傞
