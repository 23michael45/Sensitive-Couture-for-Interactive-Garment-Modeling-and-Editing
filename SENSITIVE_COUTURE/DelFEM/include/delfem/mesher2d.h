...
public:
	unsigned int id;	//!< ID
	unsigned int id_e_cad;	//!< CAD的边缘ID(如果不涉及CAD的话0)
...
public:
	unsigned int id;	//!< ID
	unsigned int id_l_cad;	//!< CAD的表面ID(如果不涉及CAD的话0)
	int ilayer;
...
public:
  ...
  virtual ~CMesher2D(){}
	...
	virtual unsigned int GetDimention() const{ return 2; }	//!< 返回坐标的维（2）
...
protected:
	...
	void MakeElemLocationType(); // 初始化元素的位置和种类（ID为数）的排列
	...
	// 如果is mistel = = true的话，max spece是无效的值
